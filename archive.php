...
<?php $secBreadcrumb = array('projects'=>'Кейсы'); ?>
            ...
            <div class="col-sm-12">
                <div class="breadcrumbs" itemscope="" itemtype="http://schema.org/BreadcrumbList">
                    <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                        <a class="breadcrumbs__link" href="https://saitcraft.ru/" itemprop="item">
                    ...
                    <span class="breadcrumbs__separator">
                        <svg width="20" height="1" viewBox="0 0 20 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <line x1="20" y1="0.25" y2="0.25" stroke="#ABA7BE" stroke-width="0.5"></line>
                        </svg>
                    </span>
                    <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                        <a class="breadcrumbs__link" href="/projects/" itemprop="item">
                    ...
                    <span class="breadcrumbs__current"><?php the_archive_title();?></span>
                </div>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-12">
                                ...
                                <?php
                                    foreach($tags as $tag) {
                                        if($tag_id == $tag->term_id){
                                            $classTag = 'tag-cloud-link active__tag';
                                        }else{
                                            $classTag = 'tag-cloud-link';
                                        }

                                        if ($tag->name == "Другое"){
                                            $other = '<li><a class="'.$classTag.'" href="'.get_term_link($tag, 'post_tag').'">'.$tag->name.'</a></li>';
                                        } elseif ($tag->name == "Строительство"){
                                            $stroi = '<li><a class="tag-cloud-link" href="'.get_term_link($tag, 'post_tag').'">'.$tag->name.'</a></li>';
                                        } else {?>
                                            <li><a class="<?php echo $classTag ?>" href="<?php echo get_term_link($tag, 'post_tag') ?>"><?php echo $tag->name ?></a></li>
                                    <?php }
                                    }
                                    echo $stroi;
                                    echo $other;
                ...
                <div class="select-tag d-sm-none">
                    <input class="select__input" type="hidden" name="">
                    <div class="select__head" data-id="<?php echo $tag_id ?>">Выберите</div>
                    <ul class="select__list" style="display: none;">
                        <li class="select__item" data-id="" data-link="/projects/">Все</li>
                        <?php foreach($tags as $tag) {
							if($tag_id == $tag->term_id){ 
								$classTag = 'select__item active';
							}else{
								$classTag = 'select__item';
							} 
							if ($tag->name == "Другое"){
								$other = '<li class="'.$classTag.'" data-id="'.$tag->term_id.'" data-link="'.get_term_link($tag, 'post_tag').'">'.$tag->name.'</li>';
							}else{?>
                        <li class="<?php echo $classTag ?>" data-id="<?php echo $tag->term_id ?>"
                            data-link="<?php echo get_term_link($tag, 'post_tag') ?>"><?php echo $tag->name ?></li>
                        <?php }	
						}
						echo $other;
					?>
...
<section class="blog">
    <div class="blog__body container">
        ...
        <ul class="blog__tags text text--medium text--white text--w-regular">
            <li class="blog__tag">
                <a href="/blog/" class="blog__tag-link">
                    Все
                </a>
            </li>
            <?php
                $postIDs = get_objects_in_term(4, 'category');
                if (!empty($postIDs) && !is_wp_error($postIDs)) {
                    $tags = wp_get_object_terms($postIDs, 'post_tag');
                    if (!empty($tags) && !is_wp_error($tags)) {
                        ?>
                        <?php foreach($tags as $tag) { ?>
                            <li class="blog__tag">
                                <a class="blog__tag-link tag-cloud-link tag-cloud-link <?php if($tag_id == $tag->term_id):?> is-active-tag<?php endif;?>" href="<?php echo get_term_link($tag, 'post_tag') ?>"><?php echo $tag->name ?></a>
                            </li>
                        <?php } ?>
                    <?php } ?>
                <?php } ?>
